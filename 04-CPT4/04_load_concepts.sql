...
*      
*  Usage: 
*  echo "EXIT" |sqlplus CPT_20120131/myPass@DEV_VOCAB @04_load_concepts.sql  
*
******************************************************************************/
SPOOL 04_load_concepts.log
-- . 
...
UPDATE dev.concept c
 set C.CONCEPT_NAME = (select st.CONCEPT_NAME from CONCEPT_STAGE st  WHERE C.CONCEPT_CODE = st.CONCEPT_CODE and st.VOCABULARY_ID = 04 ) 
 , C.VALID_END_DATE = to_date('12312099','mmddyyyy'), C.INVALID_REASON = null
 WHERE C.VOCABULARY_ID = 04
  AND NVL(c.INVALID_REASON, 'X') = ('D')      
 AND  exists (select 1 from CONCEPT_STAGE st  WHERE C.CONCEPT_CODE = st.CONCEPT_CODE and st.VOCABULARY_ID = 04 )
  ;
