...
* 
*  Usage: 
*  echo "EXIT" |sqlplus CPT_20120131/myPass@DEV_VOCAB @04_load_relations.sql 
*
******************************************************************************/
SPOOL 04_load_relations.log
-- .  	
...
AND EXISTS (SELECT 1 FROM DEV.CONCEPT c1 
        WHERE c1.VOCABULARY_ID   =   4 AND d.CONCEPT_ID_1 =   c1.CONCEPT_ID)
AND EXISTS (SELECT 1 FROM DEV.CONCEPT c2 
        WHERE c2.VOCABULARY_ID   =   4 AND d.CONCEPT_ID_2 =   c2.CONCEPT_ID)
;
