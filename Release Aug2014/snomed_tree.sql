...
select r.* from concept_relationship r, concept c1, concept c2 
where c1.concept_id=r.concept_id_1 and c2.concept_id=concept_id_2 and c1.vocabulary_id=1 and c2.vocabulary_id=1 and r.invalid_reason is null;
create view concept_stage as
select * from concept where vocabulary_id=1 and invalid_reason is null;
...
select distinct min_levels_of_separation min, d.concept_domain, c.* from concept_tree_stage a, concept_stage c, concept_domain d where a.descendant_concept_id in (4161906, 4025832, 4152995, 4220762, 40481777, 4133529) and c.concept_id=a.ancestor_concept_id and c.concept_id=d.concept_id order by min_levels_of_separation limit 100;
-- relationships up and down
select d.concept_domain, c.concept_id, c.concept_name, r.relationship_id from concept_stage c, concept_relationship_stage r, concept_domain d where r.concept_id_1 in (4152995) and c.concept_id=r.concept_id_2 and d.concept_id=c.concept_id and r.relationship_id=144;
select d.concept_domain, c.concept_id, c.concept_name, r.relationship_id from concept_stage c, concept_relationship_stage r, concept_domain d  where r.concept_id_1 in (4211845) and c.concept_id=r.concept_id_2 and d.concept_id=c.concept_id and r.relationship_id=10;
...
and (case 
	when d.concept_domain='observation' and rts.domain='observation' then 1
	when d.concept_domain='condition_occurrence' and rts.domain='condition' then 1
	when d.concept_domain='measurement' and rts.domain='lab test' then 1
	when d.concept_domain='procedure_occurrence' and rts.domain='procedure' then 1
	else 0 end)=0
