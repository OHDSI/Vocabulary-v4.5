...
CREATE TABLE TMP_REL_TYPE AS SELECT 
    SOURCEID, 
    DESTINATIONID,
       CASE
...
-- change direction of 010 (Is a) relations
DROP TABLE TMP_REL_DIR;
CREATE TABLE TMP_REL_DIR AS SELECT 
    CASE WHEN RELATIONSHIP_ID = 010 THEN DESTINATIONID ELSE SOURCEID END AS SOURCEID,
    CASE WHEN RELATIONSHIP_ID = 010 THEN SOURCEID ELSE DESTINATIONID END AS DESTINATIONID,
    RELATIONSHIP_ID AS RELATIONSHIP_ID
    FROM TMP_REL_TYPE;

-- convert source codes to concept ids
DROP TABLE TMP_REL_IDS;
CREATE TABLE TMP_REL_IDS AS 
    SELECT
	s1.CONCEPT_ID AS CONCEPT_ID_1,
        s2.CONCEPT_ID AS CONCEPT_ID_2,
	r.RELATIONSHIP_ID AS RELATIONSHIP_ID
    FROM TMP_REL_DIR r, DEV.CONCEPT s1, DEV.CONCEPT s2 
    WHERE 
	r.SOURCEID = s1.CONCEPT_CODE AND s1.VOCABULARY_ID = 1 AND
        r.DESTINATIONID = s2.CONCEPT_CODE AND s2.VOCABULARY_ID = 1;
                                
-- add results to stage
...
DELETE                                                    
      ...
      WHERE 1 = 1 AND r.RELATIONSHIP_ID = 010 AND ROWID <> (
        SELECT MIN (t.ROWID) 
        FROM CONCEPT_RELATIONSHIP_STAGE t   
        WHERE 
    	    t.RELATIONSHIP_ID = 010 AND (
    		t.CONCEPT_ID_1 = r.CONCEPT_ID_1 AND t.CONCEPT_ID_2 = r.CONCEPT_ID_2 OR 
    		t.CONCEPT_ID_2 = r.CONCEPT_ID_1 AND t.CONCEPT_ID_1 = r.CONCEPT_ID_2
    	    )
      );
commit;
