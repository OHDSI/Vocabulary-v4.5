...
UPDATE dev.concept c
 set C.CONCEPT_NAME = (select st.CONCEPT_NAME from CONCEPT_STAGE st  WHERE C.CONCEPT_CODE = st.CONCEPT_CODE and st.VOCABULARY_ID = 08 ) 
 , C.VALID_END_DATE = to_date('12312099','mmddyyyy'), C.INVALID_REASON = null
 WHERE C.VOCABULARY_ID = 08
  AND NVL(c.INVALID_REASON, 'X') = ('D')      
 AND  exists (select 1 from CONCEPT_STAGE st  WHERE C.CONCEPT_CODE = st.CONCEPT_CODE and st.VOCABULARY_ID = 08 )
  ;
