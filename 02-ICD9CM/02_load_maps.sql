...
AND     c.SOURCE_VOCABULARY_ID IN (02)
AND     c.TARGET_VOCABULARY_ID IN (01, 19)
...
--    SEQ_CONCEPT_MAP.NEXTVAL AS SOURCE_TO_CONCEPT_MAP_ID,
    sou.SOURCE_CODE, 
    ...
    TO_DATE(SUBSTR(USER, REGEXP_INSTR(REPLACE(user,'_ALL',''), '_[[:digit:]]')+1, 256),'YYYYMMDD') AS VALID_START_DATE,
    --TO_DATE('20130314','YYYYMMDD') AS VALID_START_DATE,
...
SELECT /* DISTINCT 
    ...
    WHERE  v.SOURCE_CODE = s.SOURCE_CODE 
        AND v.SOURCE_VOCABULARY_ID = s.SOURCE_VOCABULARY_ID
        AND v.MAPPING_TYPE = s.MAPPING_TYPE
        AND v.TARGET_CONCEPT_ID = s.TARGET_CONCEPT_ID
        AND v.TARGET_VOCABULARY_ID = s.TARGET_VOCABULARY_ID
    )  AS SOURCE_TO_CONCEPT_MAP_ID,
    s.SOURCE_TO_CONCEPT_MAP_ID AS SOURCE_TO_CONCEPT_MAP_ID_ORIG,*/
    s.SOURCE_CODE, 
    ...
    PRIMARY_MAP AS PRIMARY_MAP
FROM    SOURCE_TO_CONCEPT_MAP_STAGE s
 WHERE NOT EXISTS (
    SELECT 1 FROM DEV.SOURCE_TO_CONCEPT_MAP d
        ...
        AND d.TARGET_CONCEPT_ID = s.TARGET_CONCEPT_ID
        AND d.TARGET_VOCABULARY_ID = s.TARGET_VOCABULARY_ID
        AND NVL(d.PRIMARY_MAP,'X') = NVL(s.PRIMARY_MAP, 'X')
        AND NVL(d.INVALID_REASON,'X') <> 'D'
    )             
/* WHERE   1 = 1
AND SOURCE_TO_CONCEPT_MAP_ID  IS NULL
--AND c.SOURCE_CODE LIKE 'V56%'
ORDER BY 1*/
) sou
;
