...
*      
*  Usage: 
*  echo "EXIT" |sqlplus LOINC_20120914/myPass@DEV_VOCAB @06_load_concepts.sql  
*
******************************************************************************/
SPOOL 06_load_concepts.log
-- . 
...
UPDATE dev.concept c
 set C.CONCEPT_NAME = (select st.CONCEPT_NAME from CONCEPT_STAGE st  WHERE C.CONCEPT_CODE = st.CONCEPT_CODE and st.VOCABULARY_ID IN (06, 49)    ) 
 ...
 AND  exists (select 1 from CONCEPT_STAGE st  WHERE C.CONCEPT_CODE = st.CONCEPT_CODE and st.VOCABULARY_ID IN (06, 49)    )
  ;
-- DEPRECATE MISSING
UPDATE  DEV.CONCEPT c
