...
* 
*  Usage: 
*  echo "EXIT" |sqlplus RXNORM_20120131/myPass@DEV_VOCAB @07_load_relations.sql 
*
******************************************************************************/
SPOOL 07_load_relations.log
-- .  	
...
AND EXISTS (SELECT 1 FROM DEV.CONCEPT c1 
        WHERE c1.VOCABULARY_ID   IN (07, 08, 28, 32) AND d.CONCEPT_ID_1 =   c1.CONCEPT_ID)
AND EXISTS (SELECT 1 FROM DEV.CONCEPT c2 
        WHERE c2.VOCABULARY_ID   IN (07, 08, 28, 32) AND d.CONCEPT_ID_2 =   c2.CONCEPT_ID)
AND D.RELATIONSHIP_ID NOT IN ( 1, 309, 311, 313, 135, 310 ,312, 314)
--!!!AND D.RELATIONSHIP_ID NOT IN ( 285, 286) ---!!! kill it line             
